...
export default function FormSearchData({ className }) {
  ...
  const getData = async () => {
    ...
    const response = await fetch(`${globalUrl}:3001/people/role`, {
      method: 'POST',
      headers: {
    ...
    <div className={styles.wrapper}>
      <form className={className ? className : styles.form}>
        <div className={styles.container}>
          <label className={styles.labelSelect}>
            ...
            >
              <option className={styles.options} value="name">
                NOME
              </option>
              <option className={styles.options} value="cpf">
                CPF
              </option>
            </select>
          </label>
      ...
      {showResults ? (
        ...
        <div className={styles.containerUserInfos}>
          ...
          <button
            className={styles.printUserCredential}
            onClick={() => setShowUserCredentials(false)}
          >
            Imprimir Carteirinha
          </button>
          <div className={styles.containerButtons}>
            <button className={styles.findServices}>Listar Serviços</button>
            <button className={styles.addServices}>Adicionar Serviço</button>
            <button className={styles.addPayment}>Adicionar Pagamento</button>
            <button className={styles.deletePeople}>Deletar Pessoa</button>
          </div>
          <div className={styles.userInfos}>
            <span
              className={styles.fullName}
            >{`Nome: ${userInfos.fullName}`}</span>
            ...
            >{`Email: ${userInfos.contacts.email}`}</span>
            <span
              className={styles.userPhone}
            >{`Telefone: ${userInfos.contacts.phoneNumber}`}</span>
          </div>
        </div>
      )}
