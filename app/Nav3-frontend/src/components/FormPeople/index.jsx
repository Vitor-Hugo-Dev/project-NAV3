...
export default function FormPeople() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await fetch(`${globalUrl}:3001/people`, {
        method: 'POST',
        headers: {
    ...
    <form className={styles.form} onSubmit={handleSubmit(onSubmit)}>
      <div className={styles.container}>
        ...
        {errors.fullName && errors.fullName.type === 'required' && (
          <span className={styles.error}>Esse campo é obrigatório</span>
        )}
        {errors.fullName && errors.fullName.type === 'pattern' && (
          <span className={styles.error}>
            Nome inválido, digite o nome completo
          </span>
        )}
        ...
        {errors.cpf && errors.cpf.type === 'minLength' && (
          <span className={styles.error}>
            CPF inválido, digite o CPF completo
          </span>
        )}
        ...
        {errors.phoneNumber && errors.phoneNumber.type === 'pattern' && (
          <span className={styles.error}>
            Telefone inválido, digite o telefone no formato (xx) xxxxx-xxxx
          </span>
        )}
        ...
        {errors.email && errors.email.type === 'pattern' && (
          <span className={styles.error}>E-mail inválido</span>
        )}
        ...
        {errors.district && errors.district.type === 'minLength' && (
          <span className={styles.error}>
            Cidade inválida, digite a cidade completa
          </span>
        )}
        ...
        {errors.neighborhood && errors.neighborhood.type === 'minLength' && (
          <span className={styles.error}>
            Bairro inválido, digite o bairro completo
          </span>
        )}
        ...
        {errors.street && errors.street.type === 'minLength' && (
          <span className={styles.error}>
            Rua inválida, digite a rua completa
          </span>
        )}
        ...
        {errors.number && errors.number.type === 'minLength' && (
          <span className={styles.error}>
            Número inválido, digite o número completo
          </span>
        )}
        ...
        {errors.complement && errors.complement.type === 'required' && (
          <span className={styles.error}>Esse campo é obrigatório</span>
        )}
        {errors.complement && errors.complement.type === 'minLength' && (
          <span className={styles.error}>
            Complemento inválido, digite o complemento
          </span>
        )}
      </div>

      <button className={styles.button} type="submit" {...isSubmitting}>
        Enviar
      </button>
    </form>
  );
